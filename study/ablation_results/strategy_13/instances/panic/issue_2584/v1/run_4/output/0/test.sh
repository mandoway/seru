...
error_output="$("$(cueVersion 692361740f)" eval in.cue 2>&1 > /dev/null)"
expected_output='panic: runtime error: invalid memory address or nil pointer dereference'
...
output=$($(cueVersion v0.5.0) eval in.cue || exit 1)
