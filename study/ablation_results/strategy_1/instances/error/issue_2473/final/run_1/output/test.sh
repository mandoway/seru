...
error_output="$("$(cueVersion v0.6.0-alpha.2)" export in.cue 2>&1 > /dev/null)"
expected_output='cannot add field.*: was already used'
...
output=$($(cueVersion v0.5.0) export in.cue || exit 1)
